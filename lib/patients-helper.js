...
// Shared enforcement used by every phone-collecting entry point --
// public intake, and staff-facing patient create/edit and order
// create/edit. Reads the same admin-controlled phone_validation
// setting (Settings > Business info) so the rule is consistent
// everywhere rather than the public form being stricter than what
// staff can save directly. Falls back to the stricter PH-only mode if
// the setting can't be read, same fail-safe behavior as intake.js.
async function validatePhoneForSave(phone) {
  ...
  try {
    ...
    // Fall back to the stricter default.
  }

  const valid = phoneMode === 'international'
  ...
  return {
    ...
    normalizedPhone,
    mode: phoneMode,
    message: phoneMode === 'international'
      ? 'Valid phone number'
...
module.exports = { findOrCreatePatient, normalizePhone, isPhilippineMobile, looksLikePhoneNumber, validatePhoneForSave };
